...
var _find_dup_sync = function(path, hashes) {
    ...
    if (stat.isDirectory()) {
        fs.readdirSync(path).forEach(function (filename) {
            hashes = _find_dup_sync(pth.join(path, filename), hashes);
        });
    } else if (stat.isFile()) {
        var size = stat.size;
        if (hashes[size]) {
                ...
                async.series([
                    function remove_entries(cb1) {
                        fs.readdir(path, function(err, files) {
                            ...
                            async.forEach(files, function(file, cb2) { _find_dup(file, hashes, cb2); }, cb1);
                        });
                    },
                ], callback);
            } else {
...
var find_dup_sync = function(path) {
    return _find_dup_sync(path, {});
}

var find_dup_async = function(path, callback) {
    var hashes = {}
    _find_dup(path, hashes, function(err) {
        callback(err, hashes);
    });
}
