...
var _find_dup_sync = function(path, hashes) {
    ...
    if (stat.isDirectory()) {
        ...
        if (hashes[size] && self.checksum) {
            var checksum1 = crypto.createHash('sha1').update(fs.readFileSync(path), 'binary').digest('hex');

            if (hashes[size]['']) {
                var file = hashes[size][''][0];
                var checksum2 = crypto.createHash('sha1').update(fs.readFileSync(file), 'binary').digest('hex');
